...
function pms_get_retry_url( $plan_id = '' ) {
    ...
    return apply_filters( 'pms_get_retry_url', esc_url( $url ) );
...
function pms_get_abandon_url( $plan_id = '' ) {
    ...
    return apply_filters( 'pms_get_abandon_url', esc_url( $url ) );
...
function pms_get_cancel_url( $plan_id = '' ) {
    ...
    return apply_filters( 'pms_get_cancel_url', esc_url( $url ) );
...
function pms_get_renew_url( $plan_id = '' ) {
    ...
    return apply_filters( 'pms_get_renew_url', esc_url( $url ) );
 ...
 */
function pms_get_upgrade_url( $plan_id = '' ) {
    ...
    // if current plan doesn't have any upgrades
    $plan_upgrades = pms_get_subscription_plan_upgrades( (int)$plan_id );

    if( empty( $plan_upgrades ) )
        return false;

    $url = wp_nonce_url( add_query_arg( array( 'pms-action' => 'upgrade_subscription', 'subscription_id' => $member_subscription['id'], 'subscription_plan' => $plan_id ), $account_page ), 'pms_member_nonce', 'pmstkn' );

    return apply_filters( 'pms_get_upgrade_url', esc_url( $url ) );
...
function pms_account_get_tab_url( $tab, $permalink ) {
    ...
    if ( get_option( 'permalink_structure' ) && pms_get_page( 'account' ) !== false && !apply_filters( 'pms_account_rewrite_tab_urls', false ) ) {
        if ( strstr( $permalink, '?' ) ) {
