...
Class PMS_Shortcodes {
              ...
              <ul>
                  <?php foreach( $tabs as $slug => $name ) : ?>
                      <li class="pms-account-navigation-link pms-account-navigation-link--<?php echo $slug; ?>">
                          <a class="<?php echo ( $active_tab == $slug ? 'pms-account-navigation-link--active' : '' ); ?>" href="<?php echo esc_url( ( $slug != 'logout' ? pms_account_get_tab_url( $slug, pms_get_page( 'account', true ) ) : wp_logout_url( apply_filters( 'pms_member_account_logout_url', $args['logout_redirect_url'] ) ) ) ); ?>"><?php echo $name; ?></a>
                      </li>
                  <?php endforeach; ?>
              </ul>
            </nav>
    ...
    public static function login_form( $atts ) {
        ...
        if( !is_user_logged_in() ) {
            ...
            else if( !empty( $atts['register_url'] ) )
                $register_url = pms_add_missing_http( $atts['register_url'] );

            $lostpassword_url = '';
            if( pms_get_page( 'lost-password', true ) )
                $lostpassword_url = pms_get_page( 'lost-password', true );
            else if( !empty( $atts['lostpassword_url'] ) )
    ...
    public static function action_link( $atts, $content = null ) {
        ...
        foreach( $actions as $action ) {
            if( strpos( $content, '{{' . $action . '}}' ) && strpos( $content, '{{/' . $action . '}}' ) ) {
        ...
        /**
         * @var $link   HTML output
         * @var $url    Action URL
         */
        return apply_filters( 'pms_action_link', $content, $url );
    }
